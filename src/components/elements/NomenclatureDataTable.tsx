// NomenclatureDataTable.tsx — добавлен barcodeHighlightText
...
interface NomenclatureDataTableProps {
  columns: ColumnItem[];
  visibleKeys: string[];
  ...
  checkboxLeft?: number; firstColLeft?: number;
  scrollOffset?: number; headerColor?: string; selectedColor?: string; borderColor?: string;
  highlightText?: string;
  barcodeHighlightText?: string; // НОВЫЙ ПРОПС
  initialWidths?: Record<string, number>;
  onWidthsChange?: (widths: Record<string, number>) => void;
  rowContextMenuItems?: (uid: string, name: string) => ContextMenuItem[];
  requiredColumns?: Set<string>;
  onResetToBase?: () => void;
  getRowIcon: (item: any) => string | null;
  getRowIconSize?: (item: any) => number;
  getRowFontWeight?: (item: any) => number;
  getRowNameIndent?: (item: any) => number;
}
...
const NomenclatureDataTable: React.FC<NomenclatureDataTableProps> = ({
  ...
  // Автоматическое расширение колонок при поиске
  useEffect(() => {
    ...
    const matchedColumns = columns.filter(col => {
      if (!visibleKeys.includes(col.key)) return false;
      
      return data.some(item => {
        const cellText = renderCell(col.key, item);
        return cellText && cellText.toLowerCase().includes(searchLower);
      });
    });
    ...
    setCustomWidths(prev => {
      ...
      matchedColumns.forEach(col => {
        const contentWidth = getContentWidth(col);
        
        if (!next[col.key] || next[col.key] < contentWidth) {
          next[col.key] = contentWidth;
          changed = true;
        }
      });
      
      if (changed && onWidthsChange) {
        onWidthsChange(next);
      }
      
      return changed ? next : prev;
    });
  }, [highlightText, data, columns, visibleKeys, renderCell, getContentWidth, onWidthsChange]);

  // Автоматическое расширение колонок при поиске по штрихкоду/SKU
  useEffect(() => {
    if (!barcodeHighlightText || !barcodeHighlightText.trim()) return;
    
    const searchLower = barcodeHighlightText.toLowerCase();
    
    const matchedColumns = columns.filter(col => {
      if (!visibleKeys.includes(col.key)) return false;
      if (col.key !== 'barcode' && col.key !== 'sku') return false;
  ...
  }, [barcodeHighlightText, data, columns, visibleKeys, renderCell, getContentWidth, onWidthsChange]);
    ...
    <div style={{ width: tableWidth, height: tableHeight, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'visible', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none', borderRadius: 10 }}>
        ...
        {data.map((item, idx) => {
          ...
          const iconSize = getRowIconSize ? getRowIconSize(item) : 20;
          const fontWeight = getRowFontWeight ? getRowFontWeight(item) : 400;
          const nameIndent = getRowNameIndent ? getRowNameIndent(item) : 0;
          ...
          return (
            <div key={item.uid} style={{ height: rowHeight, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? selectedColor : '#FFFFFF', cursor: 'pointer', position: 'relative', borderTop: isFirst ? 'none' : `1px solid ${borderColor}`, minWidth: columnLayout.contentWidth, boxSizing: 'border-box', userSelect: 'none' }}
              ...
              {rowIcon && (
                <div style={{ position: 'absolute', left: rowIconLeft + nameIndent, display: 'flex', alignItems: 'center', justifyContent: 'center', width: ROW_ICON_BLOCK_WIDTH, height: '100%' }}>
                  <img src={rowIcon} alt="" style={{ width: iconSize, height: iconSize, flexShrink: 0 }} />
                </div>
              )}
              ...
              {columnLayout.layout.map(col => {
                ...
                const isBarcodeColumn = col.key === 'barcode' || col.key === 'sku';
                const textLeft = isNameColumn && nameIndent > 0 ? col.left + nameIndent : col.left;
                const textWidth = isNameColumn && nameIndent > 0 ? col.width - nameIndent : col.width;
                
                // Выбираем подсветку: для barcode/sku — barcodeHighlightText, для остальных — highlightText
                const highlightForCell = isBarcodeColumn && barcodeHighlightText 
                  ? barcodeHighlightText 
                  : highlightText;
                
                return (
                  <span key={col.key} style={{ position: 'absolute', left: textLeft, top: 0, height: rowHeight, display: 'flex', alignItems: 'center', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight, color: (isGrayColumn ? isGrayColumn(col.key) : false) ? '#6B7280' : '#2D4059', overflow: 'hidden', whiteSpace: 'nowrap', width: textWidth, boxSizing: 'border-box', margin: 0 }}>
                    <span 
                    ...
                    >
                      {highlightForCell ? <HighlightedText text={cellText} highlight={highlightForCell} /> : cellText}
                    </span>
                  </span>
                );
              })}
...
export default NomenclatureDataTable;