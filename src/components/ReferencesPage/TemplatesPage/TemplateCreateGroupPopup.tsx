// TemplateCreateGroupPopup.tsx — ПОЛНЫЙ ФАЙЛ (селект каталога поверх попапа)
...
const TemplateCreateGroupPopup: React.FC<TemplateCreateGroupPopupProps> = ({
  isOpen, onClose, onSubmit, isLoading, initialParentUid, initialParentName,
}) => {
  const [name, setName] = useState('');
  const [selectedParentUid, setSelectedParentUid] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setName('');
      setSelectedParentUid(initialParentUid ?? null);
      setSelectedParentName(initialParentName ?? '');
    }
  }, [isOpen, initialParentUid, initialParentName]);

  if (!isOpen) return null;

  const handleSubmit = () => {
    if (!name.trim()) return;
    onSubmit(name.trim(), selectedParentUid);
    setName('');
  };
  ...
  return (
    <>
        ...
        >
          ...
          <div style={{ marginBottom: '30px' }}>
            ...
            <div
              ...
              style={{
                width: `${fieldWidth}px`, height: '44px', borderRadius: '10px',
                border: selectedParentUid ? '1px solid #666EFE' : '1px solid rgba(102, 110, 254, 0.15)',
                backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center',
                paddingLeft: '12px', paddingRight: '12px', cursor: 'pointer',
                boxSizing: 'border-box',
              }}
            >
              <img src={selectedParentUid ? Icon32 : Icon31} alt="" style={{ width: 14.5, height: 18, flexShrink: 0 }} />
              <span style={{
                marginLeft: '12px', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis',
                whiteSpace: 'nowrap', fontFamily: 'Inter, sans-serif', fontSize: '14px',
                fontWeight: 500, color: selectedParentUid ? '#666EFE' : '#A0A3BD',
              }}>
                {selectedParentName || 'Без родителя (корневая)'}
              </span>
              <img src={selectedParentUid ? Icon42 : Icon41} alt="" style={{ width: 18, height: 18, flexShrink: 0 }} />
            </div>
          </div>
      ...
      <CatalogSelectPopup
        ...
        popupType="templateCategory"
        zIndexOverride={zIndex + 1}
      />
    </>
  );
};
