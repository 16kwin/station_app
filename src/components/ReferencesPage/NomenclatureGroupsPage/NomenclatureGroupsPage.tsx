// NomenclatureGroupsPage.tsx — ФИНАЛЬНАЯ ВЕРСИЯ
...
interface TypePurposeRowData { [key: string]: any; }
interface TypePurposeListResponse {
  columns: string[];
  data: TypePurposeRowData[];
...
const ALL_COLUMNS: ColumnItem[] = [
  ...
  { key: 'typeMaterialName', label: 'Группа учета' },
];

const REQUIRED_COLUMNS = new Set(['name', 'typeMaterialName']);
...
const NomenclatureGroupsPage = () => {
  ...
  const [responseData, setResponseData] = useState<TypePurposeListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<TypePurposeRowData | null>(null);
  ...
  const [typeMaterialMap, setTypeMaterialMap] = useState<Record<string, string>>({});
  const [isFirstInit, setIsFirstInit] = useState(false);

  const fetchData = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-purposes?userId=${USER_ID}`);
      const response = r.data as TypePurposeListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-purposes/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-purposes/settings?userId=${USER_ID}`);
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-materials?userId=${USER_ID}`);
      const items = r.data?.data || r.data || [];
      const map: Record<string, string> = {};
      items.forEach((m: any) => {
        const uid = m.uid || m.typeName;
        map[uid] = m.name || m.typeName;
      });
      setTypeMaterialMap(map);
      setFilterOptions({
        typeMaterialName: items.map((m: any) => ({ uid: m.uid || m.typeName, name: m.name || m.typeName })),
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-purposes/events`);
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-purposes/filters-settings?userId=${USER_ID}`, { filtersJson }).catch(e => console.error(e));
  }, []);

  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    const sortJson = column ? JSON.stringify({ column, direction }) : '{}';
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-purposes/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-purposes/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/type-purposes`, { name: formName.trim(), typeMaterialUid: formTypeMaterialUid || null });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-purposes/${editItem.uid}`, { name: formName.trim(), typeMaterialUid: formTypeMaterialUid || null });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-purposes/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-purposes/${uid}`);
        }
      }
      await fetchData();
  ...
  const handleDoubleClick = (uid: string, name: string) => {
    ...
    if (item) {
      ...
      setFormTypeMaterialUid(item.typeMaterialUid || '');
  ...
  const renderCell = (key: string, item: TypePurposeRowData): string => {
    const val = item[key];
  ...
  const isGrayColumn = (key: string): boolean => {
    return !['name', 'typeMaterialName'].includes(key);
  };
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterValues['typeMaterialName'] && filterValues['typeMaterialName'].size > 0) {
      result = result.filter(row => filterValues['typeMaterialName'].has(String(row['typeMaterialUid'])));
    }
    if (sortColumn) {
      result.sort((a, b) => {
  ...
  const typeMaterialsForForm = useMemo(() => {
    const items = filterOptions['typeMaterialName'] || [];
    return items;
  }, [filterOptions]);
  ...
  const filtersText = activeFilterKeys.length > 0
    ? activeFilterKeys.map(key => {
        ...
        const optionLabels = Array.from(values).map(uid => {
          // Для фильтра по группе учета используем маппинг
          if (key === 'typeMaterialName') {
            return typeMaterialMap[uid] || uid;
          }
          // Для других фильтров (если появятся) используем filterOptions
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-purposes/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-purposes/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
      ...
      link.href = url;
      link.download = 'type-purposes.pdf';
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-purposes/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'type-purposes.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-purposes/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'type-purposes.docx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Группы номенклатуры (История изменений)' : 'Справочник: Группы номенклатуры'}
        </h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 110, left: 55, right: 55, zIndex: 10 }}>
        <TableToolbar
          ...
          onFilterToggle={(key) => {
            if (key === 'typeMaterialName') fetchFilterOptions();
          }}
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Группы номенклатуры (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание группы номенклатуры</h3>
            <div>
            ...
            <div>
              ...
              <select value={formTypeMaterialUid} onChange={e => setFormTypeMaterialUid(e.target.value)} style={selectStyle}>
                <option value="">Без группы учета</option>
                {typeMaterialsForForm.map(tm => <option key={tm.uid} value={tm.uid}>{tm.name}</option>)}
              </select>
            </div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование группы номенклатуры</h3>
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Наименование</label>
