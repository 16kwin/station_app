...
const UnitsPage = () => {
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiNomenclatureMeasures, {
        name: formName.trim(),
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiNomenclatureMeasures}/${editItem.uid}`, {
        name: formName.trim(),
        description: formDescription.trim(),
      });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      for (const uid of selectedIds) {
        await AxiosService.delete(`${ConstantInfo.restApiNomenclatureMeasures}/${uid}`);
      }
      await fetchData();
            ...
            {data.map(item => {
              ...
              return (
                <div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', boxShadow: rowBoxShadow }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.name)}>
                  ...
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059', marginLeft: 67, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_DESCRIPTION - 120 }}>{item.name}</span>
