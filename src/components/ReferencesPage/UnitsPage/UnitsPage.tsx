// UnitsPage.tsx — ИСПРАВЛЕННЫЙ (дефолтная инициализация + barcodeSearch в типе)
...
const ALL_COLUMNS: ColumnItem[] = [
  ...
  { key: 'description', label: 'Расшифровка' },
];

const REQUIRED_COLUMNS = new Set(['name', 'groupName', 'description']);
...
const UnitsPage = () => {
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/measures-crud/filters-settings?userId=${USER_ID}`, { filtersJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/measures-crud`, { name: formName.trim(), description: formDescription.trim(), groupUid: formGroupUid || null });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/measures-crud/${editItem.uid}`, { name: formName.trim(), description: formDescription.trim(), groupUid: formGroupUid || null });
      await fetchData();
  ...
  const isGrayColumn = (key: string): boolean => {
    return !['name', 'groupName', 'description'].includes(key);
  };
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание единицы измерения</h3>
            <div>
            ...
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Расшифровка</label>
              <input type="text" value={formDescription} onChange={e => setFormDescription(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateSubmit(); else if (e.key === 'Escape') setShowCreatePopup(false); }} placeholder="Введите расшифровку" style={inputStyle} />
            </div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование единицы измерения</h3>
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Наименование</label>
            ...
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Расшифровка</label>
              <input type="text" value={formDescription} onChange={e => setFormDescription(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleEditSubmit(); else if (e.key === 'Escape') setShowEditPopup(false); }} placeholder="Введите расшифровку" style={inputStyle} />
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
