// StationTypesPage.tsx — ПОЛНЫЙ ФАЙЛ С ЭКСПОРТОМ
...
import TypeIcon16Black from '../../../assets/Icons/TypeIcons/TypeIcon16Black.svg';

interface StationTypeRowData { [key: string]: any; }
interface StationTypeListResponse { 
  columns: string[]; 
  data: StationTypeRowData[]; 
...
const StationTypesPage = () => {
  ...
  const [responseData, setResponseData] = useState<StationTypeListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  ...
  const [editItem, setEditItem] = useState<StationTypeRowData | null>(null);
  ...
  const fetchData = async () => { 
    try { 
      ...
      const response = r.data as StationTypeListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(ConstantInfo.restApiStationTypeColumnsSettingsSave(USER_ID), { columnsJson })
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(ConstantInfo.restApiStationTypeColumnsSettingsSave(USER_ID), { columnsJson }).catch(e => console.error(e));
  ...
  const fetchSettings = async () => { 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiStationTypeAllSettings(USER_ID)); 
  ...
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    ...
    AxiosService.patch(ConstantInfo.restApiStationTypeSortSettingsSave(USER_ID), { sortJson }).catch(e => console.error(e));
  }, []);
  
  const fetchHistory = async () => { 
    setHistoryLoading(true); 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiStationTypeEvents); 
  ...
  const renderCell = (key: string, item: StationTypeRowData): string => {
    const val = item[key];
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) { await AxiosService.delete(`${ConstantInfo.restApiStationTypes}/${deleteTargetUid}`); }
      else { for (const uid of selectedIds) { await AxiosService.delete(`${ConstantInfo.restApiStationTypes}/${uid}`); } }
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/station-types/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/station-types/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
      ...
      link.href = url;
      link.download = 'station-types.pdf';
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/station-types/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'station-types.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/station-types/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'station-types.docx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFF', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Типы станций (История изменений)' : 'Справочник: Типы станций'}
        </h1>
      </div>
      ...
      <ConfigurationPopup 
        ...
        title="Справочник: Типы станций (Настройки списка)" 
        columns={ALL_COLUMNS} 
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание типа станции</h3>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование типа станции</h3>
