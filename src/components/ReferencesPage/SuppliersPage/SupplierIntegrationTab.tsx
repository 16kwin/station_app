// SupplierIntegrationTab.tsx — ПОЛНЫЙ ФАЙЛ (DataTable + поиск + сортировка)
  ...
  const confirmDelete = async () => {
    try {
      for (const itemUid of selectedIds) {
        await AxiosService.delete(ConstantInfo.restApiSupplierDeleteIntegration(itemUid));
      }
      setSelectedIds(new Set());
      setShowDeleteConfirm(false);
      await fetchIntegrations();
      window.dispatchEvent(new CustomEvent('refreshSupplierIntegrations'));
