// SupplierRatingTab.tsx — ПОЛНЫЙ ФАЙЛ
...
const SupplierRatingTab: React.FC<CommonSupplierProps> = (props) => {
  ...
  const handleDelete = async (ratingUid: string) => {
    ...
    try { await AxiosService.delete(ConstantInfo.restApiSupplierDeleteRating(ratingUid)); await fetchRatings(); } catch (e) { console.error(e); }
  };

  const StarRating = ({ value, size = 16, interactive = false, onChange, onHover }: { value: number; size?: number; interactive?: boolean; onChange?: (v: number) => void; onHover?: (v: number) => void; }) => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      const fillPercent = Math.min(100, Math.max(0, (value - i + 1) * 100));
      const isHovered = interactive && hoverRating >= i;
      stars.push(
        <div key={i} onClick={() => interactive && onChange?.(i)} onMouseEnter={() => interactive && onHover?.(i)} onMouseLeave={() => interactive && onHover?.(0)} style={{ width: size, height: size, position: 'relative', cursor: interactive ? 'pointer' : 'default', flexShrink: 0 }}>
          <svg width={size} height={size} viewBox="0 0 20 20" fill="none" style={{ position: 'absolute', top: 0, left: 0 }}><path d="M10 1L12.39 6.53L18.18 7.27L13.92 11.37L15.09 17.23L10 14.25L4.91 17.23L6.08 11.37L1.82 7.27L7.61 6.53L10 1Z" fill={isHovered ? '#F59E0B' : '#E5E7EB'} stroke={isHovered ? '#F59E0B' : '#D1D5DB'} strokeWidth="1"/></svg>
          {fillPercent > 0 && <svg width={size} height={size} viewBox="0 0 20 20" fill="none" style={{ position: 'absolute', top: 0, left: 0, clipPath: `inset(0 ${100 - fillPercent}% 0 0)` }}><path d="M10 1L12.39 6.53L18.18 7.27L13.92 11.37L15.09 17.23L10 14.25L4.91 17.23L6.08 11.37L1.82 7.27L7.61 6.53L10 1Z" fill="#F59E0B" stroke="#F59E0B" strokeWidth="1"/></svg>}
        </div>
      );
    }
  ...
  const TABLE_WIDTH = 1665; const HEADER_HEIGHT = 54; const ROW_HEIGHT = 54; const TABLE_HEIGHT = 378;
        ...
        <div style={{ position: 'absolute', top: 83, left: 25, display: 'flex', gap: 10 }}>
          <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', border: '1.5px solid #666EFE', flexShrink: 0 }}>
            ...
            <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              ...
              : ratings.length === 0 ? <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, color: '#9CA3AF' }}>Нет отзывов</span></div>
              : ratings.map(r => (
                <div key={r.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 20, borderTop: '0.7px solid #666EFE', backgroundColor: '#FFFFFF', position: 'relative' }}>
                  ...
                  <div style={{ width: 150 }}><StarRating value={r.rating} size={14} /></div>
                  ...
                  <button onClick={() => handleDelete(r.uid)} style={{ width: 32, height: 32, borderRadius: 6, border: 'none', backgroundColor: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, flexShrink: 0 }}>
      ...
      {showAddPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowAddPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Рейтинг</label><div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><StarRating value={hoverRating || newRating} size={32} interactive onChange={(v) => setNewRating(v)} onHover={(v) => setHoverRating(v)} /><span style={{ fontFamily: 'Inter, sans-serif', fontSize: 16, fontWeight: 600, color: '#2D4059' }}>{hoverRating || newRating || 0}</span></div></div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}><button onClick={() => setShowAddPopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Отмена</button><button onClick={handleAddSubmit} disabled={isAdding || newRating === 0} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: 'none', backgroundColor: newRating > 0 && !isAdding ? '#666EFE' : '#BCC8FF', cursor: newRating > 0 && !isAdding ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#FFFFFF' }}>{isAdding ? 'Добавление...' : 'Добавить'}</button></div>
          </div>
        </div>
      )}
    </div>
