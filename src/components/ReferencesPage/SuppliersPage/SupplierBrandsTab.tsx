...
const SupplierBrandsTab: React.FC<CommonSupplierProps> = (props) => {
  ...
  const fetchBrands = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/suppliers/${uid}/brands`);
      setBrands(res.data || []);
  ...
  const filteredBrands = React.useMemo(() => {
    ...
    if (searchValue.trim()) {
      ...
      result = result.filter(brand => {
        return [brand.name].some(v => v && String(v).toLowerCase().includes(q));
      });
    }
    if (sortColumn) {
      result.sort((a, b) => {
  ...
  const isGrayColumn = (key: string): boolean => false;
...
export default SupplierBrandsTab;