// ManufacturerAssortmentTab.tsx — ПОЛНЫЙ ФАЙЛ (fitToWidth)
...
const ManufacturerAssortmentTab: React.FC<CommonManufacturerProps> = (props) => {
  ...
  const SORT_FIELDS: SortField[] = [
    ...
    { key: 'article', label: 'Артикул', iconType: '20' },
    { key: 'typeMainName', label: 'Группа учета', iconType: '20' },
  ];
  ...
  const fetchAssortment = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/nomenclature/manufacturers`);
      const manufacturers = res.data || [];
      const manufacturer = manufacturers.find((m: any) => m.uid === uid);
      if (manufacturer) {
        const materialsRes = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/nomenclature/tree`);
        const tree = materialsRes.data || [];
        const allMaterials: AssortmentItem[] = [];
        const flatten = (nodes: any[]) => {
          nodes.forEach((node: any) => {
            if (node.materials) {
              node.materials.forEach((m: any) => {
                if (m.uid) allMaterials.push({ uid: m.uid, name: m.name || m.nameMaterial || '', article: m.article || '', code: m.code || null, typeMainName: m.typeMainName || '' });
              });
            }
        ...
        for (const mat of allMaterials) {
          try {
            const matRes = await AxiosService.get(ConstantInfo.restApiNomenclatureGetMaterial(mat.uid));
            const matData = matRes.data;
            if (matData.manufacturerUid === uid) {
              filtered.push({ uid: mat.uid, name: matData.name || mat.name, article: matData.article || mat.article, code: matData.code || mat.code, typeMainName: matData.typeMainName || mat.typeMainName });
            }
          } catch (e) { /* пропускаем */ }
        }
        setItems(filtered);
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (searchValue.trim()) {
      ...
      result = result.filter(item => {
        return [String(item.code || ''), item.name, item.article, item.typeMainName].some(v => v && String(v).toLowerCase().includes(q));
      });
    }
    if (sortColumn) {
      result.sort((a, b) => {
    ...
    return result;
  }, [items, searchValue, sortColumn, sortDirection]);
  ...
  const handleSelectAll = (e: React.MouseEvent) => { e.stopPropagation(); const all = filteredItems.length > 0 && filteredItems.every(d => selectedIds.has(d.uid)); all ? setSelectedIds(new Set()) : setSelectedIds(new Set(filteredItems.map(d => d.uid))); };
...
export default ManufacturerAssortmentTab;