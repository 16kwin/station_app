...
const ManufacturersPage = () => {
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiNomenclatureManufacturers, {
        name: formName.trim(),
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiNomenclatureManufacturers}/${editItem.uid}`, {
        name: formName.trim(),
        description: formDescription.trim(),
      });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      for (const uid of selectedIds) {
        await AxiosService.delete(`${ConstantInfo.restApiNomenclatureManufacturers}/${uid}`);
      }
      await fetchData();
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ paddingTop: 35, paddingLeft: 60 }}>
        <h1 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 30, fontWeight: 'bold', color: '#2D4059', margin: 0 }}>Справочник: Производители</h1>
      </div>
