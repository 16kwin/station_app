// ManufacturerDocumentsTab.tsx — ПОЛНЫЙ ФАЙЛ (с documentChanges)
...
import type { CommonManufacturerProps, LocalManufacturerDocument, ManufacturerDocumentChange } from './ManufacturerCreatePage';
...
const ManufacturerDocumentsTab: React.FC<CommonManufacturerProps> = (props) => {
  const { 
  ...
  const SORT_FIELDS: SortField[] = [
    { key: 'documentName', label: 'Наименование', iconType: '20' },
    { key: 'createdAt', label: 'Дата', iconType: '19' },
  ];
  ...
  const fetchDocuments = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await AxiosService.get(ConstantInfo.restApiManufacturerDocuments(uid));
  ...
  useEffect(() => {
    ...
    window.addEventListener('refreshManufacturerDocuments', handler);
    return () => window.removeEventListener('refreshManufacturerDocuments', handler);
  ...
  const confirmDelete = () => {
    for (const uid of selectedIds) {
      const isLocal = localDocuments.some(d => d.localId === uid);
      if (isLocal) {
        setLocalDocuments((prev: LocalManufacturerDocument[]) => prev.filter((d: LocalManufacturerDocument) => d.localId !== uid));
      } else {
        setDocumentChanges((prev: ManufacturerDocumentChange[]) => {
  ...
  const handleAddSubmit = () => {
    ...
    setLocalDocuments((prev: LocalManufacturerDocument[]) => [...prev, { localId, documentName: formDocName.trim(), file: selectedFile }]);
  ...
  const handleEditSubmit = () => {
    ...
    if (editIsLocal) {
      setLocalDocuments((prev: LocalManufacturerDocument[]) => prev.map((d: LocalManufacturerDocument) => d.localId === editDocUid ? { ...d, documentName: editDocName.trim() } : d));
    } else {
      setDocumentChanges((prev: ManufacturerDocumentChange[]) => {
  ...
  const formatDate = (d: string) => { if (!d) return ''; try { const date = new Date(d); const dd = String(date.getDate()).padStart(2, '0'); const mm = String(date.getMonth() + 1).padStart(2, '0'); const yyyy = date.getFullYear(); const hh = String(date.getHours()).padStart(2, '0'); const min = String(date.getMinutes()).padStart(2, '0'); return `${dd}.${mm}.${yyyy} ${hh}:${min}`; } catch { return d; } };
    ...
    if (key === 'originalName') {
      return (
        <span 
          ...
          onClick={(e) => { e.stopPropagation(); handleDoubleClick(item.uid); }}
        >
          {item.originalName || '—'}
        </span>
      );
...
export default ManufacturerDocumentsTab;