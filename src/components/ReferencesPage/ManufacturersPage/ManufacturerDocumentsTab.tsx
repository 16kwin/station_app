...
import type { CommonManufacturerProps, LocalManufacturerDocument } from './ManufacturerCreatePage';
    ...
    try {
      for (const uid of selectedIds) {
        ...
        if (isLocal) {
          ...
          await AxiosService.delete(ConstantInfo.restApiManufacturerDeleteDocument(uid));
        }
      }
  ...
  const handleAddSubmit = async () => {
    ...
    if (isEdit) {
      try {
        ...
        await AxiosService.post(ConstantInfo.restApiManufacturerDocuments(uid), fd);
        await fetchDocuments();
        window.dispatchEvent(new CustomEvent('refreshManufacturerDocuments'));
      } catch (e) { console.error(e); }
    } else {
