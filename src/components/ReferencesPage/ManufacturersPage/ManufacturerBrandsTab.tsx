...
const ManufacturerBrandsTab: React.FC<CommonManufacturerProps> = (props) => {
  const { uid, isEdit } = props;

  const [brands, setBrands] = useState<ManufacturerBrandItem[]>([]);
  ...
  const SORT_FIELDS: SortField[] = [
    ...
    { key: 'description', label: 'Описание', iconType: '20' },
  ];
  ...
  const fetchBrands = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/brands-crud?userId=1`);
      const items = res.data?.data || res.data || [];
      setBrands(items.filter((b: any) => b.manufacturerUid === uid));
    } catch (e) { console.error(e); } finally { setIsLoading(false); }
  };

  useEffect(() => { if (uid && isEdit) fetchBrands(); }, [uid, isEdit]);
  ...
  const filteredBrands = React.useMemo(() => {
    ...
    if (searchValue.trim()) {
      ...
      result = result.filter(brand => {
        return [brand.name, brand.description].some(v => v && String(v).toLowerCase().includes(q));
      });
    }
    if (sortColumn) {
      result.sort((a, b) => {
    ...
    return result;
  }, [brands, searchValue, sortColumn, sortDirection]);
  ...
  const handleSelectAll = (e: React.MouseEvent) => { e.stopPropagation(); const all = filteredBrands.length > 0 && filteredBrands.every(d => selectedIds.has(d.uid)); all ? setSelectedIds(new Set()) : setSelectedIds(new Set(filteredBrands.map(d => d.uid))); };
