...
const ManufacturerEventLogTab: React.FC<CommonManufacturerProps> = (props) => {
  const { uid } = props;
  ...
  const fetchEvents = async () => {
    ...
    setIsLoading(true);
    try {
      const res = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/manufacturers-crud/${uid}/events`);
      setEvents(res.data || []);
    } catch (e) { console.error(e); } finally { setIsLoading(false); }
  };

  useEffect(() => { if (uid) fetchEvents(); }, [uid]);
  useEffect(() => { const handler = () => { if (uid) fetchEvents(); }; window.addEventListener('refreshManufacturerEvents', handler); return () => window.removeEventListener('refreshManufacturerEvents', handler); }, [uid]);
...
export default ManufacturerEventLogTab;