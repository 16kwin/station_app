// BrandsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
interface BrandRowData { [key: string]: any; }
...
const REQUIRED_COLUMNS = new Set(['name', 'manufacturerName']);
...
const BrandsPage = () => {
  ...
  const [responseData, setResponseData] = useState<BrandListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<BrandRowData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/brands-crud?userId=${USER_ID}`);
      const response = r.data as BrandListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/brands-crud/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/brands-crud/settings?userId=${USER_ID}`);
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      setFilterOptions({
        manufacturerName: items.map((m: any) => ({ uid: m.uid || m.name, name: m.name || m.typeName })),
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/brands-crud/events`);
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/brands-crud/filters-settings?userId=${USER_ID}`, { filtersJson }).catch(e => console.error(e));
  }, []);

  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    const sortJson = column ? JSON.stringify({ column, direction }) : '{}';
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/brands-crud/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/brands-crud/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/brands-crud`, {
        name: formName.trim(),
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/brands-crud/${editItem.uid}`, {
        name: formName.trim(),
        description: formDescription.trim(),
        manufacturerUid: formManufacturerUid || null,
      });
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/brands-crud/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/brands-crud/${uid}`);
        }
      }
      await fetchData();
  ...
  const renderCell = (key: string, item: BrandRowData): string => {
    const val = item[key];
  ...
  const isGrayColumn = (key: string): boolean => {
    return !['name', 'manufacturerName'].includes(key);
  };
  ...
  const manufacturersForForm = useMemo(() => {
    return filterOptions['manufacturerName'] || [];
  }, [filterOptions]);
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/brands-crud/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/brands-crud/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/brands-crud/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/brands-crud/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Бренды (История изменений)' : 'Справочник: Бренды'}
        </h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 110, left: 55, right: 55, zIndex: 10 }}>
        <TableToolbar
          ...
          onFilterToggle={(key) => {
            if (key === 'manufacturerName') fetchFilterOptions();
          }}
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Бренды (Настройки списка)"
        columns={ALL_COLUMNS}
          ...
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <select value={formManufacturerUid} onChange={e => setFormManufacturerUid(e.target.value)} style={selectStyle}>
                ...
                {manufacturersForForm.map(m => <option key={m.uid} value={m.uid}>{m.name}</option>)}
              </select>
            </div>
            <div>
