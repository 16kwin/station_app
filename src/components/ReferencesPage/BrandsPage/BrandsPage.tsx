// components/ReferencesPage/BrandsPage/BrandsPage.tsx
...
const BrandsPage = () => {
  ...
  useEffect(() => {
    setData([
      { uid: '1', name: 'SKF', manufacturer: 'SKF Group' },
      { uid: '2', name: 'FAG', manufacturer: 'Schaeffler' },
      { uid: '3', name: 'NSK', manufacturer: 'NSK Ltd' },
      { uid: '4', name: 'Gates', manufacturer: 'Gates Corp' },
      { uid: '5', name: 'Mobil', manufacturer: 'ExxonMobil' },
      { uid: '6', name: 'Shell', manufacturer: 'Shell PLC' },
    ]);
    setIsLoading(false);
  }, []);
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ paddingTop: 35, paddingLeft: 60 }}><h1 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 30, fontWeight: 'bold', color: '#2D4059', margin: 0 }}>Справочник: Бренды</h1></div>
      ...
      <div style={{ position: 'absolute', top: 159, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', border: tbs }}>
          <div style={{ height: 54, minHeight: 54, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 52, paddingRight: 40 }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 700, color: '#FFFFFF', position: 'absolute', left: 600 }}>ПРОИЗВОДИТЕЛЬ</span>
          </div>
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => (
              <div key={item.uid} style={{ height: 54, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, borderTop: cbs, backgroundColor: '#FFFFFF', cursor: 'pointer', position: 'relative' }}>
                ...
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: 600 }}>{item.manufacturer || '—'}</span>
              </div>
            ))}
          </div>
        </div>
