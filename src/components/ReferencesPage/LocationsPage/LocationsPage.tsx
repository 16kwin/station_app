// LocationsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
const LocationsPage = () => {
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/locations-crud/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/locations-crud/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/locations-crud/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/locations-crud/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
