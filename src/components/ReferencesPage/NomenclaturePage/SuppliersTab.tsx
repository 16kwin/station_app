// SuppliersTab.tsx — ПОЛНЫЙ ФАЙЛ (space-between, fitToWidth)
...
const SUPPLY_COLUMNS = [
  { key: 'supplierName', label: 'Наименование поставщика' },
  { key: 'supplyDate', label: 'Дата последней поставки' },
...
const SuppliersTab: React.FC<CommonProps> = (props) => {
  ...
  const SORT_FIELDS: SortField[] = [
    { key: 'supplierName', label: 'Наименование', iconType: '20' },
  ...
  const filteredSupplies = React.useMemo(() => {
    ...
    if (searchValue.trim()) {
      ...
      result = result.filter(s => {
        return [s.supplierName, formatDate(s.supplyDate), s.documentName].some(v => v && String(v).toLowerCase().includes(q));
      });
    }
    if (sortColumn) {
      result.sort((a, b) => {
    ...
    return result;
  }, [supplies, searchValue, sortColumn, sortDirection]);
  ...
  const handleSelectAll = (e: React.MouseEvent) => { e.stopPropagation(); const all = filteredSupplies.length > 0 && filteredSupplies.every(d => selectedIds.has(d.uid)); all ? setSelectedIds(new Set()) : setSelectedIds(new Set(filteredSupplies.map(d => d.uid))); };
  ...
  const handleContextMenu = (e: React.MouseEvent, uid: string) => {
    e.preventDefault();
    e.stopPropagation();
    const supply = supplies.find(d => d.uid === uid);
    setContextMenu({ x: e.clientX, y: e.clientY, uid, name: supply?.supplierName || '' });
  };
  
  const handleDoubleClick = (uid: string) => {
    const supply = supplies.find(x => x.uid === uid);
    if (!supply || !supply.fileUrl) return;
    const fullUrl = getFullUrl(supply.fileUrl);
  ...
  const isGrayColumn = (key: string): boolean => key !== 'supplierName';
    ...
    <div style={{ position: 'absolute', top: 165, left: 30, right: 30, bottom: 96 }}>
      ...
      <div style={{ position: 'absolute', top: 52, left: 0 }}>
        <DataTable
          ...
          visibleKeys={['supplierName', 'supplyDate', 'documentName']}
          ...
          headerHeight={58}
          firstColLeft={60}
          noWrapColumns={['supplierName', 'supplyDate', 'documentName']}
      ...
      {showAddPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowAddPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Поставщик</label><div onClick={() => setShowSupplierPopup(true)} style={{ ...popupFieldStyle, color: newSupplierName ? '#666EFE' : '#9CA3AF' }}><span>{newSupplierName || 'Выберите поставщика'}</span></div></div>
      ...
      <CatalogSelectPopup isOpen={showSupplierPopup} onClose={() => setShowSupplierPopup(false)} onSelect={(id, name) => { setNewSupplierUid(id); setNewSupplierName(name); setShowSupplierPopup(false); }} popupType="supplier" />
    </div>
  );
};
