...
const DocumentsTab: React.FC<CommonProps> = () => {
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [hasScroll, setHasScroll] = useState(false);

  const [folders] = useState([
    {
      id: 1, name: 'Документы качества', isOpen: false,
      items: [
        { id: 1, name: 'Сертификат соответствия', status: 'Действует', date: '2026-05-14 12:00' },
        { id: 2, name: 'Паспорт изделия', status: 'Действует', date: '2026-04-20 09:30' },
      ],
    },
    {
      id: 2, name: 'Техническая документация', isOpen: false,
      items: [
        { id: 3, name: 'Чертеж общего вида', status: 'Актуален', date: '2026-03-01 14:00' },
        { id: 4, name: 'Спецификация', status: 'Требует обновления', date: '2025-12-10 11:00' },
      ],
    },
  ]);
                    ...
                    {openFolders.has(folder.id) && folder.items.map(item => (
                      <motion.div key={item.id} initial={{ height: 0, opacity: 0 }} animate={{ height: 54, opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3, ease: 'easeInOut' }} style={{ overflow: 'hidden' }}>
                        <div style={{ height: 54, display: 'flex', alignItems: 'center', paddingLeft: 40, paddingRight: 40, borderTop: '0.7px solid #666EFE', backgroundColor: '#FFFFFF', cursor: 'pointer', position: 'relative' }}>
                          ...
                          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: 600 }}>{item.status}</span>
                          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', marginLeft: 'auto' }}>{item.date}</span>
                        </div>
                      </motion.div>
                    ))}
                  </AnimatePresence>
