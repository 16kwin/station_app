...
const CatalogSelectPopup: React.FC<CatalogSelectPopupProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      if (isCatalog) {
        ...
        const response = await AxiosService.get(`${ConstantInfo.restApiNomenclatureTypeProducts}?typePurposeUid=${filterParam}`);
        setData(convertFlatReference(response.data));
      } else if (popupType === 'nomenclatureType' && !filterParam) {
  ...
  const handleCreateClick = () => {
    ...
    setCreateFormDesignation('');
    setCreateFormTypeMaterialUid('');
    setCreateFormTypePurposeUid('');
    setCreateFormManufacturerUid('');
    setCreateFormBrandUid('');
    setFilteredBrands([]);
    ...
    try {
      ...
      switch (popupType) {
        ...
        case 'nomenclatureGroup':
          ...
          break;
        case 'nomenclatureType':
          url = ConstantInfo.restApiNomenclatureTypeProducts;
          body.typePurposeUid = createFormTypePurposeUid || null;
          break;
        case 'attributeType':
          url = ConstantInfo.restApiNomenclatureTypeAttributes;
          body.designation = createFormDesignation.trim();
          break;
        case 'unit':
          url = ConstantInfo.restApiNomenclatureMeasures;
          body.description = createFormDescription.trim();
          break;
        case 'manufacturer':
          url = ConstantInfo.restApiNomenclatureManufacturers;
          body.description = createFormDescription.trim();
          break;
        case 'brand':
          url = ConstantInfo.restApiNomenclatureBrands;
          body.description = createFormDescription.trim();
          body.manufacturerUid = createFormManufacturerUid || null;
          break;
        case 'model':
          url = ConstantInfo.restApiNomenclatureModels;
          body.description = createFormDescription.trim();
          body.brandUid = createFormBrandUid || null;
          break;
        case 'country':
          url = ConstantInfo.restApiNomenclatureCountries;
          break;
        case 'shortDescription':
  ...
  const handleManufacturerChange = (uid: string) => {
    setCreateFormManufacturerUid(uid);
    setCreateFormBrandUid('');
    if (uid) {
      setFilteredBrands(allBrands.filter(b => (b as any).manufacturerUid === uid));
    } else {
      setFilteredBrands([]);
    }
  };
    ...
    items.forEach((item) => {
      ...
      result.push(
        <div
          key={item.id}
          onClick={() => {
            if (isMaterial) handleItemClick(item.id, item.name);
            else if (hasChildren) toggleFolder(item.id);
          }}
        ...
        >
          {isMaterial ? (
            ...
            <img src={hasChildren ? (isOpen ? Icon12 : Icon11) : Icon11} alt="" 
    ...
    return items.map((item) => (
      <div
        ...
        style={{
          ...
          borderTop: '0.5px solid #E5ECF5',
          borderBottom: '0.5px solid #E5ECF5',
          paddingLeft: 22, paddingRight: 40,
        }}
      >
        {flatIcon && (
          <img src={flatIcon} alt="" style={{ width: 20, height: 20, flexShrink: 0 }} />
        )}
        <span style={{
          fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#2D4059',
          marginLeft: flatIcon ? 10 : 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          maxWidth: (config.columns[0]?.left || 500) - 50,
        }}>{item.name}</span>
        ...
        >
          ...
          {config.hasCreateButton && (
            <div style={{ display: 'flex', alignItems: 'center', marginTop: 30, paddingLeft: 45, paddingRight: 45 }}>
              <button style={{ width: 40, height: 40, backgroundColor: '#FFFFFF', border: '1px solid rgba(102, 110, 254, 0.15)', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
              ...
              <div style={{ marginLeft: 'auto' }}>
                <button onClick={handleCreateClick} style={{ height: 40, paddingLeft: 15, paddingRight: 15, backgroundColor: '#FFFFFF', border: '1px solid rgba(102, 110, 254, 0.15)', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', padding: 0 }}>
                  ...
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#2D4059', marginLeft: 10, marginRight: 12 }}>
          ...
          <div style={{ display: 'flex', marginTop: config.hasCreateButton ? 15 : 60, alignSelf: 'center', position: 'relative', width: TABLE_WIDTH, height: TABLE_HEIGHT }}>
            <div style={{ width: '100%', height: '100%', backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
              <div style={{ height: HEADER_HEIGHT, minHeight: HEADER_HEIGHT, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, boxSizing: 'border-box', position: 'relative' }}>
                ...
                {config.columns.map(col => (
                  <span key={col.key} style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 600, color: '#FFFFFF', position: 'absolute', left: col.left }}>
                    {col.title}
                  </span>
                ))}
              </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10003, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <input type="text" value={createFormName} onChange={e => setCreateFormName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateSubmit(); else if (e.key === 'Escape') setShowCreatePopup(false); }} placeholder="Введите название" autoFocus style={inputStyle} />
            </div>

            {popupType === 'nomenclatureGroup' && (
              <div>
                ...
                <select value={createFormTypeMaterialUid} onChange={e => setCreateFormTypeMaterialUid(e.target.value)} style={selectStyle}>
                  <option value="">Без группы учета</option>
                  {typeMaterials.map((tm: any) => <option key={tm.uid} value={tm.uid}>{tm.typeName}</option>)}
                </select>
              </div>
            )}

            {popupType === 'nomenclatureType' && (
              <div>
                ...
                <select value={createFormTypePurposeUid} onChange={e => setCreateFormTypePurposeUid(e.target.value)} style={selectStyle}>
                  ...
                  {typePurposes.map((tp: any) => <option key={tp.uid} value={tp.uid}>{tp.typeName}</option>)}
                </select>
              </div>
            )}

            {popupType === 'attributeType' && (
              <div>
                ...
                <input type="text" value={createFormDesignation} onChange={e => setCreateFormDesignation(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateSubmit(); else if (e.key === 'Escape') setShowCreatePopup(false); }} placeholder="Введите обозначение" style={inputStyle} />
              </div>
            )}
            ...
            {(popupType === 'brand' || popupType === 'model') && (
              <div>
                ...
                <select value={createFormManufacturerUid} onChange={e => popupType === 'model' ? handleManufacturerChange(e.target.value) : setCreateFormManufacturerUid(e.target.value)} style={selectStyle}>
                  <option value="">Выберите производителя</option>
                  {manufacturers.map((m: any) => <option key={m.uid} value={m.uid}>{m.name}</option>)}
                </select>
              </div>
            )}
            ...
            {popupType === 'model' && (
              <div>
                ...
                <select value={createFormBrandUid} onChange={e => setCreateFormBrandUid(e.target.value)} style={{ ...selectStyle, opacity: createFormManufacturerUid ? 1 : 0.5, cursor: createFormManufacturerUid ? 'pointer' : 'not-allowed' }} disabled={!createFormManufacturerUid}>
                  <option value="">{createFormManufacturerUid ? 'Выберите бренд' : 'Сначала выберите производителя'}</option>
                  {filteredBrands.map((b: any) => <option key={b.uid} value={b.uid}>{b.name}</option>)}
                </select>
              </div>
            )}
