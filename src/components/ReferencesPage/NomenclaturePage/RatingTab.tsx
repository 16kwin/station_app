// RatingTab.tsx — ПОЛНЫЙ ФАЙЛ (синие звёзды в попапе, редактирование через контекстное меню)
...
const RatingTab: React.FC<CommonProps> = (props) => {
  ...
  const handleContextDelete = () => {
    ...
    AxiosService.delete(ConstantInfo.restApiNomenclatureDeleteRating(contextMenu.ratingUid))
      .then(() => fetchRatings())
    ...
    <div style={{ ...cs, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ ...blockStyle, width: 1740, height: 72, flexShrink: 0, display: 'flex', alignItems: 'center', position: 'relative' }}>
        <div style={{ position: 'absolute', left: 40, display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#2D4059' }}>
            Средний рейтинг: {averageRating.toFixed(1)}
          </span>
          <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 500, color: '#666EFE', marginLeft: 42 }}>
                    ...
                    {Array.from({ length: totalRows }).map((_, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div 
                            onClick={() => handleViewComment(rating.comment, rating.author)}
                            style={{ 
                              position: 'absolute', left: COL_NAME, 
          ...
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <StarRatingSmall value={hoverRating || newRating} size={32} />
                  ...
                  {[1, 2, 3, 4, 5].map(i => {
                    ...
                    return (
                      <div
                        ...
                        style={{ width: 32, height: 32, position: 'relative', cursor: 'pointer', flexShrink: 0 }}
                      >
                        <svg width={32} height={32} viewBox="0 0 20 20" fill="none" style={{ position: 'absolute', top: 0, left: 0 }}>
                          <path d="M10 1L12.39 6.53L18.18 7.27L13.92 11.37L15.09 17.23L10 14.25L4.91 17.23L6.08 11.37L1.82 7.27L7.61 6.53L10 1Z" 
                            fill={isHovered ? '#666EFE' : '#E5E7EB'} 
                            stroke={isHovered ? '#666EFE' : '#D1D5DB'} 
                            strokeWidth="1"
                          />
                        </svg>
          ...
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Escape') setShowEditPopup(false); }}
                placeholder="Введите имя автора"
                autoFocus
                style={{ width: '100%', height: 44, borderRadius: 10, border: '1px solid rgba(102, 110, 254, 0.15)', paddingLeft: 12, paddingRight: 12, fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', outline: 'none', boxSizing: 'border-box', backgroundColor: '#FFFFFF' }}
              />
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                  {[1, 2, 3, 4, 5].map(i => {
                    const fillPercent = Math.min(100, Math.max(0, ((editHoverRating || editRating) - i + 1) * 100));
                    const isHovered = editHoverRating >= i;
                    return (
                      <div
                        key={i}
      ...
      {showViewPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowViewPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            {viewAuthor && (
              <div>
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 600, color: '#2D4059' }}>Автор: </span>
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059' }}>{viewAuthor}</span>
              </div>
            )}

            <div>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', margin: 0, lineHeight: '1.5' }}>
                {viewComment || 'Без текста'}
              </p>
            </div>

            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setShowViewPopup(false)} style={{ height: 44, paddingLeft: 24, paddingRight: 24, borderRadius: 10, border: '1px solid rgba(102,110,254,0.15)', backgroundColor: '#FFFFFF', cursor: 'pointer', fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059' }}>Закрыть</button>
            </div>
          </div>
        </div>
      )}
