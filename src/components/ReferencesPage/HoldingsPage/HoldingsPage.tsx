// HoldingsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
const HoldingsPage = () => {
  ...
  const [editItem, setEditItem] = useState<HoldingRowData | null>(null);
  ...
  const fetchData = async () => { 
    try { 
      ...
      const response = r.data as HoldingListResponse;
  ...
  const fetchSettings = async () => { 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiHoldingAllSettings(USER_ID)); 
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(ConstantInfo.restApiHoldingFiltersSettingsSave(USER_ID), { filtersJson }).catch(e => console.error(e));
  }, []);
  
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    const sortJson = column ? JSON.stringify({ column, direction }) : '{}';
    AxiosService.patch(ConstantInfo.restApiHoldingSortSettingsSave(USER_ID), { sortJson }).catch(e => console.error(e));
  }, []);
  
  const fetchHistory = async () => { 
    setHistoryLoading(true); 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiHoldingEvents); 
  ...
  useEffect(() => { fetchData(); fetchLocations(); fetchSettings(); }, []);
  ...
  const handleCreateClick = () => { setFormName(''); setFormDescription(''); setFormLocationId(''); setEditItem(null); setShowCreatePopup(true); };
  const handleCreateSubmit = async () => {
    if (!formName.trim()) return;
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiHoldings, { name: formName.trim(), description: formDescription.trim(), locationUid: formLocationId || null });
      await fetchData();
  ...
  const handleEditClick = () => {
    ...
    if (item) { setEditItem(item); setFormName(item.name); setFormDescription(item.description || ''); setFormLocationId(item.locationUid || ''); setShowEditPopup(true); }
    setContextMenu(null);
  };
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiHoldings}/${editItem.id}`, { name: formName.trim(), description: formDescription.trim(), locationUid: formLocationId || null });
      await fetchData();
  ...
  const renderCell = (key: string, item: HoldingRowData): string => {
    const val = item[key];
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        ...
        await AxiosService.delete(`${ConstantInfo.restApiHoldings}/${item?.id}`);
      } else {
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/holdings/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/holdings/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/holdings/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/holdings/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFF', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Холдинги (История изменений)' : 'Справочник: Холдинги'}
        </h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 110, left: 55, right: 55, zIndex: 10 }}>
        <TableToolbar
          ...
          onFilterToggle={(key) => { if (key === 'locationName') fetchLocations(); }}
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Холдинги (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание холдинга</h3>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование холдинга</h3>
