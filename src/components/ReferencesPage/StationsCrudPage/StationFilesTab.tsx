// StationFilesTab.tsx — ПОЛНЫЙ ФАЙЛ (локальное хранение, отправка при "Записать")
...
export interface StationLocalDocumentItem {
  localId: string;
  documentName: string;
  file: File;
  isNew?: boolean;
...
export interface StationDocumentChange {
  serverUid?: string;
...
interface StationFilesTabProps {
  stationUid: string;
  isEdit: boolean;
  localDocuments?: StationLocalDocumentItem[];
  setLocalDocuments?: React.Dispatch<React.SetStateAction<StationLocalDocumentItem[]>>;
  documentChanges?: StationDocumentChange[];
  setDocumentChanges?: React.Dispatch<React.SetStateAction<StationDocumentChange[]>>;
}
...
const StationFilesTab: React.FC<StationFilesTabProps> = ({ 
  stationUid, 
  isEdit,
  localDocuments = [],
  setLocalDocuments,
  ...
  useEffect(() => {
    if (isEdit && stationUid && documentChanges.length === 0 && localDocuments.length === 0) {
      fetchDocuments();
    }
  }, [documentChanges.length, localDocuments.length, isEdit, stationUid]);
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const serverDocs = (res.data || []).map((doc: any) => ({
        ...doc,
      ...
      setDocuments(serverDocs.filter((d: any) => !deletedServerUids.includes(d.uid)));
    } catch (e) {
  ...
  const handleEditSubmit = () => {
    ...
    if (editIsLocal) {
      ...
      setDocuments(prev => prev.map(d => d.uid === editDocUid ? { ...d, documentName: editDocName.trim() } : d));
    }
