// ProductionDirectionsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
interface ProductionDirectionRowData { [key: string]: any; }
interface ProductionDirectionListResponse {
  columns: string[];
  data: ProductionDirectionRowData[];
...
const ProductionDirectionsPage = () => {
  const { openTab } = useTabs();
  const [responseData, setResponseData] = useState<ProductionDirectionListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<ProductionDirectionRowData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/production-directions?userId=${USER_ID}`);
      const response = r.data as ProductionDirectionListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/production-directions/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/production-directions/settings?userId=${USER_ID}`);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/production-directions/events`);
  ...
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/production-directions/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/production-directions/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/production-directions`, { name: formName.trim() });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/production-directions/${editItem.uid}`, { name: formName.trim() });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/production-directions/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/production-directions/${uid}`);
        }
      }
      await fetchData();
  ...
  const renderCell = (key: string, item: ProductionDirectionRowData): string => {
    const val = item[key];
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/production-directions/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/production-directions/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
      ...
      link.href = url;
      link.download = 'production-directions.pdf';
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/production-directions/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'production-directions.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/production-directions/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'production-directions.docx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Направления производства (История изменений)' : 'Справочник: Направления производства'}
        </h1>
      </div>
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Направления производства (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание направления производства</h3>
            <div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование направления производства</h3>
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Наименование</label>
...
export default ProductionDirectionsPage;