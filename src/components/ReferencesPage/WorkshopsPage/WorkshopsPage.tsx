// WorkshopsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
const WorkshopsPage = () => {
  ...
  const [responseData, setResponseData] = useState<WorkshopListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  ...
  const [editItem, setEditItem] = useState<WorkshopRowData | null>(null);
  ...
  const fetchData = async () => { 
    try { 
      ...
      const response = r.data as WorkshopListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(ConstantInfo.restApiWorkshopColumnsSettingsSave(USER_ID), { columnsJson })
  ...
  const fetchSettings = async () => { 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiWorkshopAllSettings(USER_ID)); 
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>, placement: PlacementSelections) => {
    ...
    (Object.keys(placement) as PlacementKey[]).forEach(key => {
      if (placement[key].size > 0) placementObj[key] = Array.from(placement[key]);
    });
    if (Object.keys(placementObj).length > 0) filtersJsonObj.placementSelections = placementObj;
  ...
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    ...
    AxiosService.patch(ConstantInfo.restApiWorkshopSortSettingsSave(USER_ID), { sortJson }).catch(e => console.error(e));
  }, []);
  
  const fetchHistory = async () => { 
    setHistoryLoading(true); 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiWorkshopEvents); 
  ...
  const fetchLocations = async () => { 
    try { 
      ...
      const respData = r.data as any;
      const items = Array.isArray(respData) ? respData : (respData.data || []);
      setLocationList(items.map((item: any) => ({ uid: item.uid, name: item.name }))); 
    } catch (e) { console.error(e); } 
  };
    ...
    setVisibleColumns(prev => {
      ...
      if (changed) {
        setResponseData(prevData => {
          ...
          PLACEMENT_LEVELS.forEach(level => {
            if (!newColumns.includes(level.key)) newColumns.push(level.key);
          });
  ...
  const handleCreateClick = () => { setFormName(''); setFormDescription(''); setFormAddress(''); setFormEnterpriseId(''); setFormLocationId(''); setEditItem(null); setShowCreatePopup(true); };
  const handleCreateSubmit = async () => {
    if (!formName.trim() || !formEnterpriseId) return;
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiWorkshops, { 
        name: formName.trim(), 
        description: formDescription.trim(),
        address: formAddress.trim(),
        enterpriseId: Number(formEnterpriseId),
        locationUid: formLocationId || null,
      });
      await fetchData();
  ...
  const handleEditClick = () => {
    ...
    if (item) { 
      ...
      setFormLocationId(item.locationUid || ''); 
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiWorkshops}/${editItem.id}`, { 
        name: formName.trim(), 
        description: formDescription.trim(),
        address: formAddress.trim(),
        enterpriseId: Number(formEnterpriseId),
  ...
  const renderCell = (key: string, item: WorkshopRowData): string => {
    const val = item[key];
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        ...
        await AxiosService.delete(`${ConstantInfo.restApiWorkshops}/${item?.id}`);
      } else {
  ...
  const getSortValue = (row: WorkshopRowData, sortKey: string): string => {
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/workshops/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/workshops/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/workshops/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/workshops/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFF', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Цеха (История изменений)' : 'Справочник: Цеха'}
        </h1>
      </div>
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Цеха (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание цеха</h3>
            ...
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Адрес</label><input type="text" value={formAddress} onChange={e => setFormAddress(e.target.value)} placeholder="Введите адрес" style={inputStyle} /></div>
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Предприятие</label><select value={formEnterpriseId} onChange={e => setFormEnterpriseId(e.target.value)} style={selectStyle}><option value="">Выберите предприятие</option>{enterpriseList.map(e => (<option key={e.id} value={e.id}>{e.name}</option>))}</select></div>
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Расположение</label><select value={formLocationId} onChange={e => setFormLocationId(e.target.value)} style={selectStyle}><option value="">Без расположения</option>{locationList.map(l => (<option key={l.uid} value={l.uid}>{l.name}</option>))}</select></div>
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Описание</label><textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} placeholder="Введите описание" style={textareaStyle} /></div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование цеха</h3>
