...
interface TypeAttributeRowData { [key: string]: any; }
interface TypeAttributeListResponse {
  columns: string[];
  data: TypeAttributeRowData[];
...
const ALL_COLUMNS: ColumnItem[] = [
  ...
  { key: 'designation', label: 'Обозначение' },
  { key: 'groupName', label: 'Группа характеристик' },
];

const REQUIRED_COLUMNS = new Set(['name', 'designation', 'groupName']);
...
const AttributeTypesPage = () => {
  ...
  const [responseData, setResponseData] = useState<TypeAttributeListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<TypeAttributeRowData | null>(null);
  const [formName, setFormName] = useState('');
  const [formDesignation, setFormDesignation] = useState('');
  const [formGroupUid, setFormGroupUid] = useState('');
  ...
  const fetchData = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud?userId=${USER_ID}`);
      const response = r.data as TypeAttributeListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/settings?userId=${USER_ID}`);
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/attribute-groups?userId=${USER_ID}`);
      const items = r.data?.data || r.data || [];
      setFilterOptions({
        groupName: items.map((g: any) => ({ uid: g.uid || g.name, name: g.name || g.typeName })),
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/events`);
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/filters-settings?userId=${USER_ID}`, { filtersJson }).catch(e => console.error(e));
  }, []);

  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    const sortJson = column ? JSON.stringify({ column, direction }) : '{}';
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud`, { name: formName.trim(), designation: formDesignation.trim(), groupUid: formGroupUid || null });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/${editItem.uid}`, { name: formName.trim(), designation: formDesignation.trim(), groupUid: formGroupUid || null });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-attributes-crud/${uid}`);
        }
      }
      await fetchData();
  ...
  const renderCell = (key: string, item: TypeAttributeRowData): string => {
    const val = item[key];
  ...
  const isGrayColumn = (key: string): boolean => {
    return !['name', 'designation', 'groupName'].includes(key);
  };
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterValues['groupName'] && filterValues['groupName'].size > 0) {
      result = result.filter(row => filterValues['groupName'].has(String(row['groupUid'])));
    }
    if (sortColumn) {
      result.sort((a, b) => {
        const aVal = String(a[sortColumn] || '');
        const bVal = String(b[sortColumn] || '');
        if (sortColumn === 'name' || sortColumn === 'designation') {
  ...
  const groupsForForm = useMemo(() => {
    const items = filterOptions['groupName'] || [];
    return items;
  }, [filterOptions]);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Виды характеристик (История изменений)' : 'Справочник: Виды характеристик'}
        </h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 110, left: 55, right: 55, zIndex: 10 }}>
        <TableToolbar
          ...
          onFilterToggle={(key) => {
            if (key === 'groupName') fetchFilterOptions();
          }}
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Виды характеристик (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа характеристик</label>
              <select value={formGroupUid} onChange={e => setFormGroupUid(e.target.value)} style={selectStyle}>
                <option value="">Без группы</option>
                {groupsForForm.map(g => <option key={g.uid} value={g.uid}>{g.name}</option>)}
              </select>
            </div>
