// AttributeGroupsPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
interface AttributeGroupRowData { [key: string]: any; }
interface AttributeGroupListResponse {
  columns: string[];
  data: AttributeGroupRowData[];
...
const AttributeGroupsPage = () => {
  ...
  const [responseData, setResponseData] = useState<AttributeGroupListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<AttributeGroupRowData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = r.data as AttributeGroupListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/settings?userId=${USER_ID}`);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/events`);
  ...
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/attribute-groups`, { name: formName.trim() });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/${editItem.uid}`, { name: formName.trim() });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/attribute-groups/${uid}`);
        }
      }
      await fetchData();
  ...
  const renderCell = (key: string, item: AttributeGroupRowData): string => {
    const val = item[key];
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/attribute-groups/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/attribute-groups/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
      ...
      link.href = url;
      link.download = 'attribute-groups.pdf';
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/attribute-groups/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'attribute-groups.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/attribute-groups/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'attribute-groups.docx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Группы характеристик (История изменений)' : 'Справочник: Группы характеристик'}
        </h1>
      </div>
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Группы характеристик (Настройки списка)"
        columns={ALL_COLUMNS}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание группы характеристик</h3>
            <div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование группы характеристик</h3>
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Наименование</label>
