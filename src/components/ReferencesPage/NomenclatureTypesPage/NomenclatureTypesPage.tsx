// components/ReferencesPage/NomenclatureTypesPage/NomenclatureTypesPage.tsx
...
const NomenclatureTypesPage = () => {
  ...
  const [data, setData] = useState<NomenclatureType[]>([]);
  ...
  useEffect(() => {
    setData([
      { uid: '1', name: 'Болт', nomenclatureGroup: 'Крепёж' },
      { uid: '2', name: 'Гайка', nomenclatureGroup: 'Крепёж' },
      { uid: '3', name: 'Шариковый', nomenclatureGroup: 'Подшипники' },
      { uid: '4', name: 'Роликовый', nomenclatureGroup: 'Подшипники' },
      { uid: '5', name: 'Клиновой', nomenclatureGroup: 'Ремни' },
      { uid: '6', name: 'Зубчатый', nomenclatureGroup: 'Ремни' },
      { uid: '7', name: 'Масло', nomenclatureGroup: 'Смазки' },
    ]);
    setIsLoading(false);
  }, []);
      ...
      <div style={{ position: 'absolute', top: 159, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', border: tableBorderStyle }}>
          <div style={{ height: 54, minHeight: 54, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 52, paddingRight: 40 }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 700, color: '#FFFFFF', position: 'absolute', left: 600 }}>ГРУППЫ НОМЕНКЛАТУРЫ</span>
          </div>
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => (
              <div key={item.uid} style={{ height: 54, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, borderTop: cellBorderStyle, backgroundColor: '#FFFFFF', cursor: 'pointer', position: 'relative' }}>
                ...
                <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: 600 }}>{item.nomenclatureGroup || '—'}</span>
              </div>
            ))}
          </div>
        </div>
