// NomenclatureTypesPage.tsx — ИСПРАВЛЕННЫЙ (добавлены печать, PDF, Excel, Word)
...
interface TypeProductRowData { [key: string]: any; }
interface TypeProductListResponse {
  columns: string[];
  data: TypeProductRowData[];
...
const REQUIRED_COLUMNS = new Set(['name', 'typePurposeName']);
...
const NomenclatureTypesPage = () => {
  ...
  const [responseData, setResponseData] = useState<TypeProductListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
  const [showCreatePopup, setShowCreatePopup] = useState(false);
  const [showEditPopup, setShowEditPopup] = useState(false);
  const [editItem, setEditItem] = useState<TypeProductRowData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-products?userId=${USER_ID}`);
      const response = r.data as TypeProductListResponse;
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-products/columns-settings?userId=${USER_ID}`, { columnsJson })
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-products/settings?userId=${USER_ID}`);
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      setFilterOptions({
        typePurposeName: items.map((p: any) => ({ uid: p.uid || p.name, name: p.name || p.typeName })),
      });
    } catch (e) {
      console.error(e);
    }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const r = await AxiosService.get(`${ConstantInfo.apiBaseUrl}/api/type-products/events`);
  ...
  const saveFilters = useCallback((filters: Record<string, Set<string>>) => {
    ...
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-products/filters-settings?userId=${USER_ID}`, { filtersJson }).catch(e => console.error(e));
  }, []);

  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    const sortJson = column ? JSON.stringify({ column, direction }) : '{}';
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-products/sort-settings?userId=${USER_ID}`, { sortJson }).catch(e => console.error(e));
  }, []);
  ...
  const handleColumnWidthsChange = useCallback((widths: Record<string, number>) => {
    ...
    const columnsJson = JSON.stringify(columnsJsonObj);
    AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-products/columns-settings?userId=${USER_ID}`, { columnsJson }).catch(e => console.error(e));
  ...
  const handleCreateSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.post(`${ConstantInfo.apiBaseUrl}/api/type-products`, { name: formName.trim(), typePurposeUid: formTypePurposeUid || null });
      await fetchData();
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.apiBaseUrl}/api/type-products/${editItem.uid}`, { name: formName.trim(), typePurposeUid: formTypePurposeUid || null });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-products/${deleteTargetUid}`);
      } else {
        for (const uid of selectedIds) {
          await AxiosService.delete(`${ConstantInfo.apiBaseUrl}/api/type-products/${uid}`);
        }
      }
      await fetchData();
  ...
  const renderCell = (key: string, item: TypeProductRowData): string => {
    const val = item[key];
  ...
  const isGrayColumn = (key: string): boolean => {
    return !['name', 'typePurposeName'].includes(key);
  };
  ...
  const filteredData = useMemo(() => {
    ...
    if (filterValues['typePurposeName'] && filterValues['typePurposeName'].size > 0) {
      result = result.filter(row => filterValues['typePurposeName'].has(String(row['typePurposeUid'])));
    }
    if (sortColumn) {
      result.sort((a, b) => {
  ...
  const typePurposesForForm = useMemo(() => {
    const items = filterOptions['typePurposeName'] || [];
    return items;
  }, [filterOptions]);
  ...
  const handlePrint = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-products/print`,
        preparePayload(),
        { responseType: 'blob' }
      );
  ...
  const handleDownloadPdf = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-products/export-pdf`,
        preparePayload(),
        { responseType: 'blob' }
      );
      const pdfBlob = new Blob([res.data], { type: 'application/pdf' });
      ...
      link.href = url;
      link.download = 'type-products.pdf';
  ...
  const handleDownloadExcel = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-products/export-excel`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'type-products.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  const handleDownloadWord = async () => {
    try {
      const res = await AxiosService.post(
        `${ConstantInfo.apiBaseUrl}/api/type-products/export-word`,
        preparePayload(),
        { responseType: 'blob' }
      );
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = 'type-products.docx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Виды номенклатуры (История изменений)' : 'Справочник: Виды номенклатуры'}
        </h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 110, left: 55, right: 55, zIndex: 10 }}>
        <TableToolbar
          ...
          onFilterToggle={(key) => {
            if (key === 'typePurposeName') fetchFilterOptions();
          }}
      ...
      <ConfigurationPopup
        ...
        title="Справочник: Виды номенклатуры (Настройки списка)"
        columns={ALL_COLUMNS}
          ...
          <div style={{ width: 400, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              <select value={formTypePurposeUid} onChange={e => setFormTypePurposeUid(e.target.value)} style={selectStyle}>
                <option value="">Без группы номенклатуры</option>
                {typePurposesForForm.map(tp => <option key={tp.uid} value={tp.uid}>{tp.name}</option>)}
              </select>
            </div>
