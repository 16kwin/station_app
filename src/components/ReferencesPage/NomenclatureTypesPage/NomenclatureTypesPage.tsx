// NomenclatureTypesPage.tsx — полный файл с каскадным выбором
...
const NomenclatureTypesPage = () => {
  ...
  // Все группы номенклатуры
  const [allTypePurposes, setAllTypePurposes] = useState<TypePurposeOption[]>([]);
  // Отфильтрованные группы номенклатуры для выбранной группы учета
  ...
  const loadReferences = async () => {
    try {
      const [materialsRes, purposesRes] = await Promise.all([
      ...
      setTypeMaterials(materialsRes.data || []);
      setAllTypePurposes(purposesRes.data || []);
    } catch (error) {
  ...
  useEffect(() => {
    if (activeTabId && activeTabId === tabIdRef.current && data.length > 0) {
      fetchData();
    }
  }, [activeTabId]);
  ...
  // Фильтрация групп номенклатуры при смене группы учета
  const handleTypeMaterialChange = (materialUid: string) => {
    setFormTypeMaterialUid(materialUid);
    setFormTypePurposeUid(''); // сбрасываем группу номенклатуры
    
    if (materialUid) {
      const filtered = allTypePurposes.filter(p => (p as any).typeMaterialUid === materialUid);
      setFilteredTypePurposes(filtered);
    } else {
      setFilteredTypePurposes([]);
    }
  };
  ...
  const handleCreateClick = () => {
    setFormName('');
    setFormTypeMaterialUid('');
    setFormTypePurposeUid('');
    setFilteredTypePurposes([]);
    setEditItem(null);
  ...
  const handleCreateSubmit = async () => {
    ...
    try {
      await AxiosService.post(ConstantInfo.restApiNomenclatureTypeProducts, {
        name: formName.trim(),
        typePurposeUid: formTypePurposeUid || null,
      });
      await fetchData();
  ...
  const handleEditClick = () => {
    ...
    if (item) {
      setEditItem(item);
      setFormName(item.typeName);
      
      // Найти группу учета по группе номенклатуры
      const purpose = allTypePurposes.find(p => p.uid === item.typePurposeUid);
      const materialUid = purpose ? (purpose as any).typeMaterialUid || '' : '';
      
      setFormTypeMaterialUid(materialUid);
      setFormTypePurposeUid(item.typePurposeUid || '');
      
      // Загрузить отфильтрованные группы номенклатуры
      if (materialUid) {
        const filtered = allTypePurposes.filter(p => (p as any).typeMaterialUid === materialUid);
        setFilteredTypePurposes(filtered);
      } else {
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiNomenclatureTypeProducts}/${editItem.uid}`, {
        name: formName.trim(),
        typePurposeUid: formTypePurposeUid || null,
      });
      await fetchData();
  ...
  const confirmDelete = async () => {
    try {
      for (const uid of selectedIds) {
        await AxiosService.delete(`${ConstantInfo.restApiNomenclatureTypeProducts}/${uid}`);
      }
      await fetchData();
  ...
  const HeaderEmptySquare = ({ isSelected, onClick }: { isSelected: boolean; onClick: (e: React.MouseEvent) => void }) => (
    <div onClick={(e) => { e.stopPropagation(); onClick(e); }} style={{ width: 18, height: 18, borderRadius: 2, border: isSelected ? 'none' : '2px solid #FFFFFF', opacity: isSelected ? 1 : 1, flexShrink: 0, boxSizing: 'border-box', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {isSelected && <img src={Icon19} alt="" style={{ width: 18, height: 18 }} />}
    </div>
  );
  ...
  // Общий стиль для select в попапах
  ...
  return (
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFC' }}>
      <div style={{ paddingTop: 35, paddingLeft: 60 }}>
        <h1 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 30, fontWeight: 'bold', color: '#2D4059', margin: 0 }}>Справочник: Виды номенклатуры</h1>
      </div>
      ...
      <div style={{ position: 'absolute', top: 159, left: 40 }}>
        <div style={{ width: TABLE_WIDTH, height: TABLE_HEIGHT, backgroundColor: '#F5F6FA', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', border: tableBorderStyle }}>
          <div style={{ height: HEADER_HEIGHT, minHeight: HEADER_HEIGHT, backgroundColor: '#666EFE', borderTopLeftRadius: 8, borderTopRightRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 20, paddingRight: 40, position: 'relative' }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 700, color: '#FFFFFF', position: 'absolute', left: COL_GROUP }}>ГРУППА НОМЕНКЛАТУРЫ</span>
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 700, color: '#FFFFFF', position: 'absolute', left: COL_ACCOUNT }}>ГРУППА УЧЕТА</span>
          </div>
          <div ref={scrollContainerRef} style={{ flex: 1, overflowY: 'auto', overflowX: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {data.map(item => {
              ...
              return (
                <div key={item.uid} style={{ height: ROW_HEIGHT, display: 'flex', alignItems: 'center', backgroundColor: isSelected ? '#EDF6FF' : '#FFFFFF', cursor: 'pointer', position: 'relative', boxShadow: rowBoxShadow }} onContextMenu={(e) => handleContextMenu(e, item.uid, item.typeName)}>
                  ...
                  <img src={Icon72} alt="" style={{ width: 16, height: 16, flexShrink: 0, marginLeft: 18 }} />
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 15, fontWeight: 400, color: '#2D4059', marginLeft: 16, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_GROUP - 120 }}>{item.typeName}</span>
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: COL_GROUP, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: COL_ACCOUNT - COL_GROUP - 20 }}>{item.typePurposeName || '—'}</span>
                  <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, color: '#2D4059', position: 'absolute', left: COL_ACCOUNT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: TABLE_WIDTH - COL_ACCOUNT - 60 }}>{item.typeMaterialName || '—'}</span>
                </div>
              );
            })}
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание вида номенклатуры</h3>
            ...
            <div>
              ...
              <input type="text" value={formName} onChange={e => setFormName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreateSubmit(); else if (e.key === 'Escape') setShowCreatePopup(false); }} placeholder="Введите название" autoFocus style={{ width: '100%', height: 44, borderRadius: 10, border: '1px solid rgba(102, 110, 254, 0.15)', paddingLeft: 12, paddingRight: 12, fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            
            {/* Группа учета (сначала) */}
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа учета</label>
              <select value={formTypeMaterialUid} onChange={e => handleTypeMaterialChange(e.target.value)} style={selectStyle}>
                <option value="">Выберите группу учета</option>
                {typeMaterials.map(tm => <option key={tm.uid} value={tm.uid}>{tm.typeName}</option>)}
              </select>
            </div>
            
            {/* Группа номенклатуры (зависит от группы учета) */}
            <div>
              <label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Группа номенклатуры</label>
              <select 
                value={formTypePurposeUid} 
                onChange={e => setFormTypePurposeUid(e.target.value)} 
                style={{ ...selectStyle, opacity: formTypeMaterialUid ? 1 : 0.5, cursor: formTypeMaterialUid ? 'pointer' : 'not-allowed' }}
                disabled={!formTypeMaterialUid}
              >
                <option value="">{formTypeMaterialUid ? 'Выберите группу' : 'Сначала выберите группу учета'}</option>
                {filteredTypePurposes.map(tp => <option key={tp.uid} value={tp.uid}>{tp.typeName}</option>)}
              </select>
            </div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование вида номенклатуры</h3>
