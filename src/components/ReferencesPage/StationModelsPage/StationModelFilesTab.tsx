// StationModelFilesTab.tsx — ПОЛНЫЙ ФАЙЛ (top: 0 для корректного позиционирования)
...
const StationModelFilesTab: React.FC<StationModelFilesTabProps> = ({ 
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      setServerDocuments(serverDocs.filter((d: any) => !deletedServerUids.includes(d.uid)).map((doc: any) => ({
        uid: doc.uid,
        documentName: doc.documentName,
        originalName: doc.originalName,
        url: doc.url ? (doc.url.startsWith('http') ? doc.url : ConstantInfo.fileDir + doc.url.replace(/^\//, '')) : '',
