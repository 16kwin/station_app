...
import ManufacturerIcon18Black from '../../../assets/Icons/ManufacturerIcons/ManufacturerIcon18Black.svg';

interface EnterpriseRowData { [key: string]: any; }
interface EnterpriseListResponse { 
  columns: string[]; 
  data: EnterpriseRowData[]; 
...
const EnterprisesPage = () => {
  ...
  const [responseData, setResponseData] = useState<EnterpriseListResponse>({ columns: [], data: [], columnWidths: {}, requiredColumns: [] });
  ...
  const [editItem, setEditItem] = useState<EnterpriseRowData | null>(null);
  const [formName, setFormName] = useState('');
  const [formDescription, setFormDescription] = useState('');
  const [formAddress, setFormAddress] = useState('');
  const [formHoldingId, setFormHoldingId] = useState<string>('');
  ...
  useEffect(() => {
    if (isFirstInit && !isLoading) {
      ...
      AxiosService.patch(ConstantInfo.restApiEnterpriseColumnsSettingsSave(USER_ID), { columnsJson })
  ...
  const saveSort = useCallback((column: string | null, direction: 'asc' | 'desc') => {
    ...
    AxiosService.patch(ConstantInfo.restApiEnterpriseSortSettingsSave(USER_ID), { sortJson }).catch(e => console.error(e));
  }, []);
  
  const fetchHistory = async () => { 
    setHistoryLoading(true); 
    try { 
      const r = await AxiosService.get(ConstantInfo.restApiEnterpriseEvents); 
  ...
  const handleCreateClick = () => { setFormName(''); setFormDescription(''); setFormAddress(''); setFormHoldingId(''); setFormLocationId(''); setEditItem(null); setShowCreatePopup(true); };
  const handleCreateSubmit = async () => {
    if (!formName.trim()) return;
    setIsSaving(true);
    try {
      await AxiosService.post(ConstantInfo.restApiEnterprises, { 
        name: formName.trim(), 
  ...
  const handleEditClick = () => {
    ...
    if (item) { 
      ...
      setFormHoldingId(item.holdingId ? String(item.holdingId) : ''); 
  ...
  const handleEditSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await AxiosService.patch(`${ConstantInfo.restApiEnterprises}/${editItem.id}`, { 
        name: formName.trim(), 
        description: formDescription.trim(), 
        address: formAddress.trim(),
        holdingId: formHoldingId ? Number(formHoldingId) : null,
  ...
  const renderCell = (key: string, item: EnterpriseRowData): string => {
    const val = item[key];
  ...
  const confirmDelete = async () => {
    try {
      if (deleteTargetUid) {
        ...
        await AxiosService.delete(`${ConstantInfo.restApiEnterprises}/${item?.id}`);
      } else {
    ...
    <div style={{ position: 'relative', height: '100%', backgroundColor: '#FAFBFF', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: 35, left: 60 }}>
        <h1 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 600, color: '#2D4059', margin: 0, lineHeight: '29px' }}>
          {showHistory ? 'Справочник: Предприятия (История изменений)' : 'Справочник: Предприятия'}
        </h1>
      </div>
          ...
          {showHistory ? (
            ...
            <motion.div key="data" initial={{ x: 'calc(-100% - 40px)' }} animate={{ x: 0 }} exit={{ x: 'calc(-100% - 40px)' }} transition={{ duration: 0.3, ease: 'easeInOut' }} style={{ position: 'absolute', top: 0, left: 0, right: 0 }}>
              <DataTable
                ...
                rowIcon={ManufacturerIcon18Black}
              />
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {showCreatePopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowCreatePopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Создание предприятия</h3>
            ...
            <div><label style={{ fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 500, color: '#2D4059', display: 'block', marginBottom: 7 }}>Холдинг</label><select value={formHoldingId} onChange={e => setFormHoldingId(e.target.value)} style={selectStyle}><option value="">Без холдинга</option>{holdingList.map(h => (<option key={h.id} value={h.id}>{h.name}</option>))}</select></div>
      ...
      {showEditPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', backgroundColor: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(8px)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowEditPopup(false)}>
          <div style={{ width: 450, backgroundColor: '#FFFFFF', borderRadius: 20, padding: 30, boxShadow: '0 8px 32px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', gap: 20 }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Roboto, sans-serif', fontSize: 20, fontWeight: 500, color: '#2D4059', margin: 0, textAlign: 'center' }}>Редактирование предприятия</h3>
