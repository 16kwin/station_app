// DashboardHeader.tsx — шапка панели: заголовок, переключатель блока, пилюля диапазона дат, кнопки «Фильтр» и «Куб»
...
interface DashboardHeaderProps {
  /** Текущий диапазон — показывается в пилюле как «01.01.2025 - 30.11.2025» */
  range: DateRange;
  /** Клик по пилюле дат (кроме крестика); передаётся положение пилюли — якорь для попапа */
  onDateClick: (anchor: AnchorRect) => void;
  /** Клик по крестику в пилюле — сброс диапазона на значение по умолчанию */
  onDateReset: () => void;
}

/** Блоки панели; стрелки переключателя листают список по кругу (пока блок один) */
const BLOCKS = ['Экономический блок'];

// Геометрия шапки в координатах холста 1800×840
const TITLE_LEFT = 60;
const TITLE_TOP = 36;
const SWITCHER = { left: 753, top: 38, width: 294, height: 54 } as const;
const SWITCHER_ARROW_OFFSET = 27; // центр шеврона от края пилюли
const ARROW_BUTTON_SIZE = 40;
const DATE_PILL = { left: 1374, top: 52, width: 266, height: 40 } as const;
const DATE_ICON_LEFT = 14;
const DATE_TEXT_LEFT = 43;
const DATE_CROSS_LEFT = 244; // левый край крестика 8×8
const CROSS_BUTTON_SIZE = 24;
const TOOL_TOP = 52;
const TOOL_SIZE = 40;
const FILTER_LEFT = 1660;
const CUBE_LEFT = 1720;
...
/** Прозрачная кнопка 40×40 со стрелкой переключателя блока; `left` — от левого края пилюли */
const switcherArrowStyle = (left: number): React.CSSProperties => ({
  position: 'absolute',
  left,
  top: (SWITCHER.height - ARROW_BUTTON_SIZE) / 2,
...
const toolButtonStyle = (left: number): React.CSSProperties => ({
  position: 'absolute',
  left,
  top: TOOL_TOP,
  width: TOOL_SIZE,
...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ range, onDateClick, onDateReset }) => {
  const [blockIndex, setBlockIndex] = useState(0);

  const handlePrevBlock = () => setBlockIndex(i => (i - 1 + BLOCKS.length) % BLOCKS.length);
  const handleNextBlock = () => setBlockIndex(i => (i + 1) % BLOCKS.length);
  ...
  return (
    <>
      ...
      >
        Информационная панель (Топ-менеджмент)
      </div>
      ...
      >
        <button
          ...
          style={switcherArrowStyle(SWITCHER_ARROW_OFFSET - ARROW_BUTTON_SIZE / 2)}
        >
          <Chevron direction="left" />
        </button>
        <span style={{ fontFamily: FONT, fontSize: 16, fontWeight: 500, lineHeight: '19px', color: COLORS.text, whiteSpace: 'nowrap' }}>
          {BLOCKS[blockIndex]}
        </span>
        <button
          type="button"
          ...
          style={switcherArrowStyle(SWITCHER.width - SWITCHER_ARROW_OFFSET - ARROW_BUTTON_SIZE / 2)}
        >
          <Chevron direction="right" />
        </button>
      </div>

      {/* Пилюля диапазона дат: клик — попап выбора, крестик — сброс */}
      <div
        role="button"
      ...
      >
        <div style={{ position: 'absolute', left: DATE_ICON_LEFT, top: (DATE_PILL.height - 16) / 2, width: 14, height: 16, display: 'flex' }}>
          <CalendarIcon />
        </div>
        <div
          style={{
        ...
        <button
          ...
          style={{
            position: 'absolute',
            left: DATE_CROSS_LEFT + 4 - CROSS_BUTTON_SIZE / 2,
            top: DATE_PILL.height / 2 - CROSS_BUTTON_SIZE / 2,
            width: CROSS_BUTTON_SIZE,
            height: CROSS_BUTTON_SIZE,
            border: 'none',
            padding: 0,
            background: 'transparent',
            cursor: 'pointer',
      ...
      {/* Кнопка «Фильтр» — пока без функции */}
      <button type="button" aria-label="Фильтр" style={toolButtonStyle(FILTER_LEFT)}>
        <img src={FilterIcon18Black} alt="" draggable={false} style={{ width: 18, height: 18 }} />
      </button>

      {/* Кнопка «Куб» — пока без функции */}
      <button type="button" aria-label="Куб" style={toolButtonStyle(CUBE_LEFT)}>
        <CubeIcon />
      </button>
    </>
  );
