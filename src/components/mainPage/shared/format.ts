...
/** 1820 → "1 820" — целое количество с пробелом между тысячами */
export const formatCount = (value: number): string => formatRubSpaces(value);

/** 17.81 → "18%" — доля, округлённая до целого (как требует ТЗ по уровню брака) */
export const formatPercentInt = (value: number): string => `${Math.round(Number.isFinite(value) ? value : 0)}%`;

/** 8000 → "8000"; подпись оси Y для графика расхода объема */
export const formatAxisCount = (value: number): string => String(Math.round(value));

/** '2026-05-12T17:40:00' → '12.05.2026 17:40' */
export const formatDateTimeRu = (value: string): string => {
